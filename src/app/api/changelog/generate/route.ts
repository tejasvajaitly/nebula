...
export async function GET(req: Request) {
  ...
  const commitMessages = await fetchCommits(url);
...
async function fetchCommits(url: URL) {
  ...
  const commitResponse = await fetch(
    `${baseUrl}/api/changelog/fetchCommits?repo_id=${repoId}&repo_name=${repoName}&repo_owner=${repoOwner}&date_from=${dateFrom}&date_to=${dateTo}&github_account_id=${githubAccountId}`
  );
  const commitData = await commitResponse.json();

  return commitData.commitMessages;
}
