...
function GenerateChangelog({
  ...
  const fetchChangelog = async () => {
    const response = await fetch(
      `/api/changelog/first-changelog?repo_id=${selectedRepo?.id}&repo_name=${
        selectedRepo?.name
      }&repo_owner=${
        selectedRepo?.owner.login
      }&date_from=${dateFrom?.toISOString()}&github_account_id=${selectedGitHubAccountId}`
    );
    ...
    return (
      ...
      >
        ...
        <CardContent>
          ...
          >
            {mutation.isPending && <Spinner />} 🎉 Go to Dashboard
          </Button>
        </CardContent>
      </Card>
    );
